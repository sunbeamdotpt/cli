...
use sdk::kube::{get_client, kube_exec, parse_target};
use sdk::{debug, info};
...
async fn kube_secret(ns: &str, name: &str, key: &str) -> String {
    sdk::kube::kube_get_secret_field(ns, name, key)
        .await
        .unwrap_or_default()
}
...
async fn check_sso_gateway_oidc(domain: &str, client: &reqwest::Client) -> CheckResult {
    ...
    match http_get(client, &url, None).await {
        ...
        Err(e) => CheckResult::fail("sso-gateway-oidc", "auth", "sso-gateway", &e),
    }
}
...
async fn check_sso_gateway_scim(domain: &str, client: &reqwest::Client) -> CheckResult {
    ...
    match http_get(client, &url, None).await {
        Ok((status, body)) => {
            ...
            CheckResult {
                name: "sso-gateway-scim".into(),
                ns: "auth".into(),
                svc: "sso-gateway".into(),
                passed: ok_flag,
                detail,
            }
        }
        Err(e) => CheckResult::fail("sso-gateway-scim", "auth", "sso-gateway", &e),
    }
}
...
fn check_registry() -> Vec<CheckEntry> {
    vec![
        ...
        CheckEntry {
            func: |d, c| Box::pin(check_sso_gateway_oidc(d, c)),
            ns: "auth",
            svc: "sso-gateway",
        },
        CheckEntry {
            func: |d, c| Box::pin(check_sso_gateway_scim(d, c)),
            ns: "auth",
            svc: "sso-gateway",
        },
        CheckEntry {
...
pub async fn cmd_check(logger: &sdk::logger::Logger, target: Option<&str>) -> Result<()> {
...
mod tests {
    ...
    fn test_check_registry_has_all_checks() {
        ...
        assert_eq!(registry[6].ns, "auth");
        assert_eq!(registry[6].svc, "sso-gateway");
        assert_eq!(registry[7].ns, "auth");
        assert_eq!(registry[7].svc, "sso-gateway");
    ...
    fn test_check_registry_filter_namespace() {
        ...
        let filtered: Vec<&CheckEntry> = all.iter().filter(|e| e.ns == "auth").collect();
    ...
    fn test_check_registry_filter_service() {
        ...
        let filtered: Vec<&CheckEntry> = all
            .iter()
            .filter(|e| e.ns == "auth" && e.svc == "sso-gateway")
            .collect();
        assert_eq!(filtered.len(), 2);
    }
    ...
    fn test_check_registry_expected_namespaces() {
        ...
        for expected in &["devtools", "data", "storage", "auth", "media"] {
            assert!(
    ...
    #[test]
    fn test_svc_filter_auth_sso_gateway() {
        let selected = filter_registry(Some("auth"), Some("sso-gateway"));
        assert_eq!(selected.len(), 2);
        assert_eq!(selected[0], ("auth", "sso-gateway"));
    }
    ...
    fn test_filter_ns_match_svc_mismatch_returns_empty() {
        // auth namespace exists but postgres service does not live there
        let selected = filter_registry(Some("auth"), Some("postgres"));
        assert!(selected.is_empty());
    }
    ...
    #[tokio::test]
    async fn test_http_get_status_and_body() {
        ...
        Mock::given(method("GET"))
            ...
            .respond_with(ResponseTemplate::new(200).set_body_string("{\"version\":\"1.2.3\"}"))
            .mount(&server)
            .await;
        ...
        let (status, body) = http_get(
            &client,
            &format!("{}/api/v1/version", server.uri()),
            Some(&[("x-test", "yes")]),
        )
        .await
        .unwrap();
    ...
    #[tokio::test]
    async fn test_http_get_error_on_unreachable() {
        let client = reqwest::Client::builder()
            .timeout(Duration::from_millis(200))
            .build()
            .unwrap();
        let err = http_get(&client, "http://127.0.0.1:1/nope", None)
            .await
            .unwrap_err();
        assert!(!err.is_empty());
    }
    ...
    #[test]
    fn test_build_http_client_works_without_mkcert() {
        // Whether or not mkcert is installed, the builder must succeed.
