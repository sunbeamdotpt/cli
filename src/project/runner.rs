...
mod tests {
    ...
    #[test]
    fn resolve_cwd_defaults_and_joins() {
        let root = Path::new("/proj");
        assert_eq!(resolve_cwd(root, None), PathBuf::from("/proj"));
        assert_eq!(resolve_cwd(root, Some("")), PathBuf::from("/proj"));
        assert_eq!(resolve_cwd(root, Some(".")), PathBuf::from("/proj"));
        assert_eq!(
            resolve_cwd(root, Some("sub/dir")),
            PathBuf::from("/proj/sub/dir")
        );
    }

    #[test]
    fn merged_env_target_wins_over_extra() {
        let extra = BTreeMap::from([("A".to_string(), "extra".to_string())]);
        let target = BTreeMap::from([
            ("A".to_string(), "target".to_string()),
            ("B".to_string(), "target-only".to_string()),
        ]);
        let merged = merged_env(&extra, &target);
        assert_eq!(merged.get("A").unwrap(), "target");
        assert_eq!(merged.get("B").unwrap(), "target-only");
    }

    #[test]
    fn exec_display_shell_and_argv() {
        assert_eq!(
            exec_display(&ExecCommand::Shell("cargo build".into())),
            "cargo build"
        );
        assert_eq!(
            exec_display(&ExecCommand::Argv(vec!["cargo".into(), "build".into()])),
            "cargo build"
        );
    }

    #[test]
    fn build_workflow_inputs_combines_root_env_and_inputs() {
        let inputs = BTreeMap::from([("name".to_string(), serde_yaml::Value::from("x"))]);
        let env = BTreeMap::from([("FOO".to_string(), "bar".to_string())]);
        let out = build_workflow_inputs(Path::new("/proj"), &inputs, &env);
        assert_eq!(out["__project_root"], "/proj");
        assert_eq!(out["env_FOO"], "bar");
        assert_eq!(out["name"], "x");
    }

    #[tokio::test]
    async fn run_exec_empty_shell_errors() {
        let tmp = TempDir::new().unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Shell("   ".into()),
            cwd: None,
            env: BTreeMap::new(),
        };
        let err = run_exec("build", tmp.path(), &target, &RunOptions::default())
            .await
            .unwrap_err();
        assert!(
            err.to_string().contains("empty shell command"),
            "err: {err}"
        );
    }

    #[tokio::test]
    async fn run_exec_empty_argv_errors() {
        let tmp = TempDir::new().unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Argv(vec![]),
            cwd: None,
            env: BTreeMap::new(),
        };
        let err = run_exec("build", tmp.path(), &target, &RunOptions::default())
            .await
            .unwrap_err();
        assert!(err.to_string().contains("empty argv"), "err: {err}");
    }

    #[tokio::test]
    async fn run_exec_failing_exit_status_errors() {
        let tmp = TempDir::new().unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Shell("exit 3".into()),
            cwd: None,
            env: BTreeMap::new(),
        };
        let err = run_exec("build", tmp.path(), &target, &RunOptions::default())
            .await
            .unwrap_err();
        assert!(err.to_string().contains("exit status 3"), "err: {err}");
    }

    #[tokio::test]
    async fn run_exec_spawn_failure_errors() {
        let tmp = TempDir::new().unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Argv(vec!["definitely-not-a-binary-xyz".into()]),
            cwd: None,
            env: BTreeMap::new(),
        };
        let err = run_exec("build", tmp.path(), &target, &RunOptions::default())
    ...
    #[tokio::test]
    async fn run_exec_dry_run_skips_execution() {
        let tmp = TempDir::new().unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Shell("exit 1".into()),
            cwd: None,
            env: BTreeMap::new(),
        };
        let opts = RunOptions {
        ...
        let out = run_exec("build", tmp.path(), &target, &opts).await.unwrap();
        assert_eq!(out, RunOutcome::Ran);
    }

    #[tokio::test]
    async fn run_exec_respects_cwd_and_env() {
        let tmp = TempDir::new().unwrap();
        let sub = tmp.path().join("sub");
        fs::create_dir(&sub).unwrap();
        let target = ExecTarget {
            exec: ExecCommand::Shell("test \"$MARKER\" = \"hello\" && pwd | grep -q sub$".into()),
            cwd: Some("sub".into()),
            env: BTreeMap::from([("MARKER".to_string(), "hello".to_string())]),
        };
        let out = run_exec("build", tmp.path(), &target, &RunOptions::default())
            .await
            .unwrap();
        assert_eq!(out, RunOutcome::Ran);
    }
