...
async fn build_proxy_image(logger: &sdk::logger::Logger) -> wfe_core::Result<PathBuf> {
...
async fn import_image_into_k3s(tar_path: &Path, target_ref: &str) -> wfe_core::Result<()> {
    ...
    // Import the image.
    let import_result = sdk::kube::kube_exec(
        "default",
        pod_name,
        &[
            "/usr/local/bin/ctr",
    ...
    let list_result = sdk::kube::kube_exec(
        "default",
        pod_name,
        &["/usr/local/bin/ctr", "-n", "k8s.io", "images", "list", "-q"],
        None,
    ...
    let tag_result = sdk::kube::kube_exec(
        "default",
        pod_name,
        &[
            "/usr/local/bin/ctr",
