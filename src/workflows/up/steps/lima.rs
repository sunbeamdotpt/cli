...
async fn start_lima_vm() -> Result<(), String> {
    let status = tokio::process::Command::new("limactl")
        .args(["start", sdk::constants::LIMA_VM_NAME])
        .status()
        .await
...
async fn merge_kubeconfigs(
    ...
    let ctx_name = sdk::constants::LIMA_KUBE_CONTEXT;
    ...
    merged.current_context = Some(sdk::constants::LIMA_KUBE_CONTEXT.to_string());
