...
impl Default for MintVpnPreAuthKeys {
    fn default() -> Self {
        Self {
            logger: sdk::logger::Logger::new(sdk::logger::TracingSink),
        }
    }
}
...
async fn router_secret_has_key() -> sdk::error::Result<bool> {
...
async fn ensure_headscale_user(pod: &str, user: &str) -> sdk::error::Result<()> {
    ...
    if rc != 0 && !out.contains("already exists") {
        return Err(sdk::error::SunbeamError::Other(format!(
...
async fn mint_preauth_key(
    ...
    parse_preauth_key(&stdout).ok_or_else(|| {
        sdk::error::SunbeamError::Other(format!(
...
async fn write_router_secret(key: &str) -> sdk::error::Result<()> {
...
/// is already set.
fn write_user_config_key(key: &str) -> sdk::error::Result<()> {
...
mod tests {
    ...
    fn mint_vpn_preauth_keys_is_default() {
        let _ = MintVpnPreAuthKeys {
            logger: sdk::logger::Logger::new(sdk::logger::NoopSink),
        };
    }
