...
use sdk::info;
use sdk::logger::{Logger, TracingSink};
...
fn is_transient_connection_err(e: &sdk::error::SunbeamError) -> bool {
...
impl StepBody for ApplyManifest {
    ...
    async fn run(&mut self, ctx: &StepExecutionContext<'_>) -> wfe_core::Result<ExecutionResult> {
        ...
        let live_domain = data
            ...
            .and_then(|v| v.as_str())
            .filter(|s| !s.is_empty());
        let domain = live_domain.unwrap_or(&sdk::config::active_context().domain);
        ...
        let overrides: sdk::manifest_params::Overrides = data
        ...
        for attempt in 1..=5 {
            match sdk::manifests::apply_manifests(&self.logger, &opts).await {
                Ok(_) => {
                    if attempt > 1 {
                        self.logger
...
mod tests {
    ...
    #[test]
    fn is_transient_connection_err_matches_known_patterns() {
        for msg in [
            ...
            let e = sdk::error::SunbeamError::Other(msg.to_string());
            assert!(is_transient_connection_err(&e), "should match: {msg}");
        }
    }

    #[test]
    fn is_transient_connection_err_rejects_permanent_errors() {
        let e = sdk::error::SunbeamError::Other("invalid manifest yaml".to_string());
        assert!(!is_transient_connection_err(&e));
    }

    #[test]
    fn stagger_millis_is_deterministic_and_bounded() {
        let a = stagger_millis_for("ory");
        assert_eq!(a, stagger_millis_for("ory"));
        assert!(a <= 2000, "stagger out of range: {a}");
        assert_eq!(stagger_millis_for("ory") % 500, 0);
    }
}
