...
impl StepContext {
    ...
    pub async fn kube_client(&self) -> Result<kube::Client> {
        sdk::kube::get_client().await
    }

    /// Build an OpenBao HTTP client from a local port and token.
    pub fn bao_client(&self, port: u16, token: &str) -> sdk::openbao::BaoClient {
        sdk::openbao::BaoClient::with_token(&format!("http://127.0.0.1:{port}"), token)
    }
}
...
mod tests {
    ...
    fn test_from_config_empty_context_name_defaults() {
        let ctx = sdk::config::Context::default();
