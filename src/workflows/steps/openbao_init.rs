...
use chrono::Utc;
use sdk::secrets;
use sdk::vault_keystore::{VaultKeystore, keystore_exists, load_keystore, save_keystore};
...
impl StepBody for InitOrUnsealOpenBao {
    async fn run(&mut self, ctx: &StepExecutionContext<'_>) -> wfe_core::Result<ExecutionResult> {
        ...
        let status = status.unwrap_or(sdk::openbao::SealStatusResponse {
            initialized: false,
            sealed: true,
        });
        ...
        let status = bao
            .seal_status()
            .await
            .unwrap_or(sdk::openbao::SealStatusResponse {
                initialized: true,
                sealed: true,
            });
