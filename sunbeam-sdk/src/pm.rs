...
/// Carddetailresponse.
    ...
    impl Card {
        pub fn into_ticket(self, base_url: &str, included: Option<&BoardIncluded>) -> Ticket {
...
impl PlankaClient {
        ...
        let tickets = included
            ...
            .map(|inc| {
                inc.cards
                    .clone()
                    .into_iter()
                    .map(|c: planka_json::Card| c.into_ticket(&self.base_url, Some(inc)))
                    .collect()
            })
            .unwrap_or_default();
    ...
    async fn get_card(&self, id: &str) -> Result<Ticket> {
        ...
        Ok(body
            .item
            .into_ticket(&self.base_url, body.included.as_ref()))
    }
    ...
    async fn create_card(
        ...
        Ok(card
            .item
            .into_ticket(&self.base_url, card.included.as_ref()))
    }
    ...
    async fn resolve_user_id(&self, query: &str) -> Result<String> {
        ...
        if query == "me" || query == "self" {
            ...
            if let Ok(resp) = self
                ...
                && let Some(memberships) = body
                    .get("included")
                    .and_then(|i| i.get("boardMemberships"))
                    .and_then(|b| b.as_array())
                && let Some(user_id) = memberships
                    .first()
...
/// Repository.
    ...
    impl Issue {
        pub fn into_ticket(self, base_url: &str, org: &str, repo: &str) -> Ticket {
...
impl GiteaClient {
    ...
    async fn list_issues_inner(
        ...
        match repo {
            Some(r) => {
                ...
                Ok(issues
                    .into_iter()
                    .map(|i| i.into_ticket(&self.base_url, org, r))
                    .collect())
            }
            None => {
    ...
    async fn get_issue(&self, org: &str, repo: &str, index: u64) -> Result<Ticket> {
        ...
        Ok(issue.into_ticket(&self.base_url, org, repo))
    }
...
pub async fn cmd_pm_close(id: &str) -> Result<()> {
    ...
    match ticket_ref {
        TicketRef::Planka(card_id) => {
            ...
            let _ticket = client.get_card(&card_id).await?;
...
mod tests {
    ...
    fn test_planka_list_name_to_status() {
        // Test via Card::into_ticket with synthetic included data.
        use planka_json::*;
        ...
        assert_eq!(
            make_card(1).into_ticket("https://x/api", Some(&inc)).status,
            Status::Open
        );
        assert_eq!(
            make_card(2).into_ticket("https://x/api", Some(&inc)).status,
            Status::InProgress
        );
        assert_eq!(
            make_card(3).into_ticket("https://x/api", Some(&inc)).status,
            Status::Done
        );
        assert_eq!(
            make_card(4).into_ticket("https://x/api", Some(&inc)).status,
            Status::Closed
        );
    }

    #[test]
    fn test_gitea_issue_into_ticket() {
        ...
        let ticket = issue.into_ticket("https://src.example.com/api/v1", "studio", "app");
