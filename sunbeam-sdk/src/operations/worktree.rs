...
pub fn new(branch: &str, from: Option<&str>, run_setup: bool) -> Result<PathBuf> {
    ...
    // Materialize a per-worktree CLAUDE.local.md so the assistant immediately
    // sees which worktree it's in. Skip if the file already exists (user may
    // have customized it). Gitignored at the workspace root.
    if let Err(e) = write_worktree_claude_md(&target, &sanitized, from) {
        tracing::warn!("could not write CLAUDE.local.md in {target_str}: {e}");
    }
...
/// Write a per-worktree `CLAUDE.local.md` with a branch banner if absent.
///
/// The file is gitignored. It tells assistants (Claude Code, etc.) which
/// worktree they are operating in so changes don't accidentally cross
/// branches.
fn write_worktree_claude_md(target: &Path, branch: &str, from: Option<&str>) -> Result<()> {
    use std::io::Write;
    let path = target.join("CLAUDE.local.md");
    if path.exists() {
        return Ok(());
    }
    let from = from.unwrap_or("HEAD");
    let body = render_worktree_claude_md(branch, from, target);
    let mut f = std::fs::File::create(&path)
        .with_ctx(|| format!("creating {}", path.display()))?;
    f.write_all(body.as_bytes())
        .with_ctx(|| format!("writing {}", path.display()))?;
    Ok(())
}

fn render_worktree_claude_md(branch: &str, from: &str, target: &Path) -> String {
    format!(
        "# Worktree: `{branch}`\n\
         \n\
         You are operating inside the **`{branch}`** worktree, branched from `{from}`.\n\
         Path: `{path}`\n\
         \n\
         Changes here apply ONLY to this worktree's branch. Other worktrees,\n\
         the main checkout, and other branches are unaffected. Commits land on\n\
         `{branch}`. Verify with `git status` / `git branch --show-current` if\n\
         you're unsure.\n\
         \n\
         This file is auto-generated by `sunbeam wt new` and gitignored.\n\
         Customize freely; it won't be overwritten on subsequent runs.\n",
        branch = branch,
        from = from,
        path = target.display(),
    )
...
/// if no name is given. Also seeds `CLAUDE.local.md` if absent.
pub fn setup(branch: Option<&str>) -> Result<()> {
    let (target, branch_name) = match branch {
        None => {
            let cwd = std::env::current_dir()?;
            let name = std::fs::canonicalize(&cwd)
                .ok()
    ...
    if let Err(e) = write_worktree_claude_md(&target, &branch_name, None) {
        tracing::warn!("could not write CLAUDE.local.md: {e}");
    }
    run_setup_hook(&target)
}
